--[[
    SpyHandler_X9A v2.2 SOURCE

    SpyHandler_X9A is a lightweight penetration testing tool that logs remote calls.
...
-- shuts down the previous instance of SpyHandler_X9A
if _G.SpyHandler_X9AExecuted and type(_G.SpyHandler_X9AShutdown) == "function" then
	print(pcall(_G.SpyHandler_X9AShutdown))
end
...
local Highlight =
	loadstring(
		game:HttpGet("https://github.com/exxtremestuffs/SpyHandler_X9ASource/raw/master/highlight.lua?nocache=1")
	)()
...
local GuiModule_X9A = Instance.new("ScreenGui")
...
GuiModule_X9A.Name = "GuiModule_X9A"
GuiModule_X9A.ResetOnSpawn = false

Background.Name = "Background"
Background.Parent = GuiModule_X9A
...
--- True = --[[hookfunction removed]] function(f) return f end, false = namecall
local toggle = false
local gm
local original
...
local remoteLogs = {}
--- used for --[[hookfunction removed]] function(f) return f end
...
-- whether or not SpyHandler_X9A uses 'getcallingscript()' to get the script (default is false because detection)
local useGetCallingScript = false

--- used to enable/disable SpyHandler_X9A's keyToString for remotes
...
--- @param args any[]
--- @return string
function SpyHandler_X9A:ArgsToString(method, args)
...
--- @param t any[]
--- @return string
function SpyHandler_X9A:TableToVars(t)
...
--- @param value any
--- @return string
function SpyHandler_X9A:ValueToVar(value, variablename)
...
--- @param value any
--- @return string
function SpyHandler_X9A:ValueToString(value)
	return v2s(value)
end
...
--- @param func function
--- @return string
function SpyHandler_X9A:GetFunctionInfo(func)
	...
	return v2v({ functionInfo = {
		info = --[[removed debug.getinfo]] function() return {} end(func),
		constants = --[[removed debug.getconstants]] function() return {} end(func),
	} })
end

--- Gets the ScriptSignal for a specified remote being fired
--- @param remote Instance
function SpyHandler_X9A:GetRemoteFiredSignal(remote)
...
--- @param remote Instance
--- @param f function
function SpyHandler_X9A:HookRemote(remote, f)
	assert(typeof(remote) == "Instance", "Instance expected, got " .. typeof(remote))
...
--- @param remote any
function SpyHandler_X9A:BlockRemote(remote)
	assert(
...
--- @param remote any
function SpyHandler_X9A:ExcludeRemote(remote)
	assert(
		typeof(remote) == "Instance" or typeof(remote) == "string",
		"Instance | string expected, got " .. typeof(remote)
	)
...
--- Executed when the toggle button (the SpyHandler_X9A logo) is hovered over
...
--- Called when mouse enters SpyHandler_X9A
function mouseEntered()
	local existingCursor = GuiModule_X9A:FindFirstChild("Cursor")
	while existingCursor do
		existingCursor:Destroy()
		existingCursor = GuiModule_X9A:FindFirstChild("Cursor")
	end
	...
	customCursor.Image = ""
	customCursor.Parent = GuiModule_X9A
	UserInputService.OverrideMouseIconBehavior = Enum.OverrideMouseIconBehavior.ForceHide
	RunService:BindToRenderStep("SIMPLESPY_CURSOR", 1, function()
		if mouseInGui and _G.SpyHandler_X9AExecuted then
...
function genScript(remote, args)
	...
	if #args > 0 then
		...
		if not remote:IsDescendantOf(game) and not getnilrequired then
			gen = "function getNil(name,class) for _,v in pairs(--[[removed getnilinstances]] {})do if v.ClassName==class and v.Name==name then return v;end end end\n\n"
				.. gen
		end
	...
	gen = "-- Script generated by SpyHandler_X9A - credits to exx#9394\n\n" .. gen
	prevTables = {}
	return gen
end
...
function v2v(t)
	...
	if getnilrequired then
		topstr = "function getNil(name,class) for _,v in pairs(--[[removed getnilinstances]] {})do if v.ClassName==class and v.Name==name then return v;end end end\n"
			.. topstr
	end
	if #topstr > 0 then
		ret = topstr .. "\n" .. ret
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
	local globalIndex = table.find(--[[removed getgenv()]] nil, t) -- checks if table is a global
	...
	for k, v in pairs(t) do -- iterates over table
		...
		if size > (_G.SpyHandler_X9AMaxTableSize or 1000) then
			s = s
				.. "\n"
				.. string.rep(" ", l)
				.. "-- MAXIMUM TABLE SIZE REACHED, CHANGE '_G.SpyHandler_X9AMaxTableSize' TO ADJUST MAXIMUM SIZE "
			break
		end
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[Potentially hidden data (tostring in SpyHandler_X9A:HookRemote/GetRemoteFiredSignal at your own risk)]]',
				safetostring(v)
			)
		elseif typeof(v) == "userdata" then
...
--- function-to-string
function f2s(f)
	for k, x in pairs(--[[removed getgenv()]] nil) do
		local isgucci, gpath
		...
		if isgucci and type(k) ~= "function" then
			if type(k) == "string" and k:match("^[%a_]+[%w_]*$") then
				...
				return "--[[removed getgenv()]] nil[" .. v2s(k) .. "]" .. gpath
			end
		end
	end
	if funcEnabled and --[[removed debug.getinfo]] function() return {} end(f).name:match("^[%a_]+[%w_]*$") then
		return "function()end --[[" .. --[[removed debug.getinfo]] function() return {} end(f).name .. "]]"
	end
...
function formatstr(s, indentation)
	...
	return '"'
		...
		.. (
			reachedMax
				and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE '_G.SpyHandler_X9AMaxStringSize' TO ADJUST MAXIMUM SIZE ]]"
			or ""
		)
end
...
function getScriptFromSrc(src)
	...
	if src:sub(1, 1) == "=" then
		...
		for _, v in pairs(--[[removed getnilinstances]] {}) do
...
--- Handles remote logs
function remoteHandler(--[[hookfunction removed]] function(f) return f end, methodName, remote, args, funcInfo, calling, returnValue)
	...
	if validInstance and validClass then
		...
		if func and islclosure(func) then
			...
			pcall(function()
				functionInfo.constants = --[[removed debug.getconstants]] function() return {} end(func)
			end)
			pcall(function()
...
function hookRemote(remoteType, remote, ...)
	if typeof(remote) == "Instance" then
		...
		if validInstance and not (blacklist[remote] or blacklist[remoteName]) then
			...
			local calling
			if funcEnabled then
				funcInfo = --[[removed debug.getinfo]] function() return {} end(4) or funcInfo
...
local newnamecall = newcclosure(function(remote, ...)
	if typeof(remote) == "Instance" then
		...
		then
			...
			local calling
			if funcEnabled then
				funcInfo = --[[removed debug.getinfo]] function() return {} end(3) or funcInfo
				calling = useGetCallingScript and getcallingscript() or nil
			end
	...
	if not toggle then
		if hookmetamethod then
			...
			if not original then
				warn("SpyHandler_X9A: namecall method not found!")
				onToggleButtonClick()
				return
			end
		...
		originalEvent = --[[hookfunction removed]] function(f) return f end(remoteEvent.FireServer, newFireServer)
		originalFunction = --[[hookfunction removed]] function(f) return f end(remoteFunction.InvokeServer, newInvokeServer)
	else
		if hookmetamethod then
			if original then
		...
		--[[hookfunction removed]] function(f) return f end(remoteEvent.FireServer, originalEvent)
		--[[hookfunction removed]] function(f) return f end(remoteFunction.InvokeServer, originalFunction)
	end
end

--- Toggles between the two remotespy methods (--[[hookfunction removed]] function(f) return f end currently = disabled)
...
function shutdown()
	...
	_G.SpyHandler_X9AExecuted = false
end

-- main
if not _G.SpyHandler_X9AExecuted then
	local succeeded, err = pcall(function()
		if not RunService:IsClient() then
			error("SpyHandler_X9A cannot run on the server!")
		end
		if
			not --[[hookfunction removed]] function(f) return f end
			or not getrawmetatable
			or getrawmetatable and not getrawmetatable(game).__namecall
			or not setreadonly
		then
			local missing = {}
			if not --[[hookfunction removed]] function(f) return f end then
				table.insert(missing, "--[[hookfunction removed]] function(f) return f end")
			end
			...
			shutdown()
			error(
				"This environment does not support method hooks!\n(Your exploit is not capable of running SpyHandler_X9A)\nMissing: "
					.. table.concat(missing, ", ")
			)
		end
		_G.SpyHandler_X9AShutdown = shutdown
		...
		codebox:setRaw("")
		--[[removed getgenv()]] nil.SpyHandler_X9A = SpyHandler_X9A
		--[[removed getgenv()]] nil.getNil = function(name, class)
			for _, v in pairs(--[[removed getnilinstances]] {}) do
		...
		if syn and syn.protect_gui then
			pcall(syn.protect_gui, GuiModule_X9A)
		end
		bringBackOnResize()
		GuiModule_X9A.Parent = --[[gethui and gethui() or]]
			CoreGui
		_G.SpyHandler_X9AExecuted = true
	...
	if not succeeded then
		warn(
			"A fatal error has occured, SpyHandler_X9A was unable to launch properly.\nPlease DM this error message to @exx#9394:\n\n"
				.. tostring(err)
		)
		GuiModule_X9A:Destroy()
		--[[hookfunction removed]] function(f) return f end(remoteEvent.FireServer, originalEvent)
...
----- ADD ONS ----- (easily add or remove additonal functionality to the LogHook_X9A!)
--[[
    Some helpful things:
...
newButton("Get Script", function()
	...
	if selected then
		setclipboard(SpyHandler_X9A:ValueToString(selected.Source))
		TextLabel.Text = "Done!"
	end
end)
...
newButton("Function Info", function()
	...
	if selected then
		if selected.Function then
			codebox:setRaw(
				"-- Calling function info\n-- Generated by the SpyHandler_X9A serializer\n\n" .. tostring(selected.Function)
			)
		end
		TextLabel.Text = "Done! Function info generated by the SpyHandler_X9A Serializer."
	end
end)
...
--- Excludes the selected.Log Remote from the LogHook_X9A
newButton("Exclude (i)", function()
	return "Click to exclude this Remote.\nExcluding a remote makes SpyHandler_X9A ignore it, but it will continue to be usable."
end, function()
	if selected then
...
--- Excludes all Remotes that share the same name as the selected.Log remote from the LogHook_X9A
...
newButton("Block (i)", function()
	return "Click to stop this remote from firing.\nBlocking a remote won't remove it from SpyHandler_X9A logs, but it will not continue to fire the server."
end, function()
	if selected then
...
--- Prevents all remotes from firing that share the same name as the selected.Log remote from the LogHook_X9A (still logged)
...
newButton("CallingScript", function()
	return string.format(
		"[%s] [UNSAFE] Uses 'getcallingscript' to get calling script for Decompile and GetScript. Much more reliable, but opens up SpyHandler_X9A to detection and/or instability.",
...
newButton("GetReturnValue", function()
	...
	if selected then
		codebox:setRaw(SpyHandler_X9A:ValueToVar(selected.ReturnValue, "returnValue"))
	end
end)
